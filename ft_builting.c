...
int ft_mycd(info_t *info)
{
	...
	if (!s)
		ft_putstr("TODO: >>getcwd failure emsg here<<\n");
	if (!info->argv[1])
	{
	...
	else if (ft_strcmp(info->argv[1], "-") == 0)
	{
		...
		ft_putstr(ft_getenv(info, "OLDPWD=")), ft_putchar('\n');
		chdir_ret = /* TODO: what should this be? */
	...
	if (chdir_ret == -1)
	{
		...
		ft_errputstr(info->argv[1]), ft_errputchar('\n');
	}
	else
	{
...
int ft_myhelp(info_t *info)
{
	...
	arg_array = info->argv;
	ft_putstr("help call works. Function not yet implemented \n");
	if (0)
		ft_putstr(*arg_array); /* temp att_unused workaround */
	return (0);
}
